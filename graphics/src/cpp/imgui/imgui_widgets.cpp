...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    // The library position stored in popup_pos will be used by Begin() to find a suitable position for the child menu,
    ...
    if (menu_is_open)
    {
        SetNextWindowPos(popup_pos, ImGuiCond_Always); // Note: this is super misleading! The value will serve as library for FindBestWindowPosForPopup(), not actual pos.
        menu_is_open = BeginPopupEx(id, flags); // menu_is_open can be 'false' when the popup is completely clipped (e.g. zero size display)
    }
    else
    {
