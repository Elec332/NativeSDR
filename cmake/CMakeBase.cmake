...
function(useLibrary name)
    ...
    cmake_parse_arguments(ULB "PUBLIC;PRIVATE" "INSTALL" "" ${ARGN})
    ...
    if(ULB_INSTALL)
        install(IMPORTED_RUNTIME_ARTIFACTS ${name} RUNTIME DESTINATION ${ULB_INSTALL})
    endif()
endfunction()
...
function(createSharedLibrary name)
    ...
    if(CSL_COMPONENTS)
        ...
        elseif(${COMPONENTS_LEN} EQUAL 1)
            set(t_name ${name}::${CSL_COMPONENTS})
        endif()
    endif()
    ...
    if(MSVC)
        ...
        message(FATAL_ERROR "Failed to locate library: ${t_name}")
    else()
...
function(useLibInternal name)
    if(CSL_USE)
        if(CSL_INSTALL AND WIN32)
            useLibrary(${name} ${PUB} INSTALL ${CSL_INSTALL})
        else()
            useLibrary(${name} ${PUB})
        endif()
    elseif(CSL_INSTALL AND WIN32)
    endif()
endfunction()
