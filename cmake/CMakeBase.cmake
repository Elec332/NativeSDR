...
macro(processAssets)
    ...
    if(EXISTS ${PROJECT_SOURCE_DIR}/assets/)
        ...
        file(MAKE_DIRECTORY ${${PROJECT_NAME}_BINARY_DIR}/${ASSET_DIR}) ##Pre-make asset dir to fix naming errors on Linux systems
        set(ASSET_OUTPUTS)
        foreach(file ${files})
...
function(setRunPath location)
    if(NOT WIN32)
        get_property(link_flags TARGET ${PROJECT_NAME} PROPERTY LINK_FLAGS)
        set(link_flags "${link_flags} -Wl,-rpath=${location}")
...
function(useLibraries name)
    ...
    cmake_parse_arguments(ULB "PUBLIC;PRIVATE;STATIC_HEADERS" "INSTALL;WIN_INSTALL;HEADER_ROOT_DIR;PREFIX;BIN_PREFIX" "NAMES;HEADER_INCLUDE_DIRS;HEADER_INCLUDE_FILES" ${${ARG_START}})
    ...
    foreach(target ${ULB_NAMES})
        ...
        if(ULB_BIN_PREFIX)
            getConfigurableProperty(ILBINL ${target} IMPORTED_LOCATION NO_CRASH)
            if(NOT (${CONF_PROP_FULL} STREQUAL NOTFOUND) AND NOT EXISTS ${ILBINL})
                message("${target} IMPORTED_LOCATION does not exist, attempting to locate...")
                get_filename_component(ULB_BIN_PREFIX "${ULB_BIN_PREFIX}" ABSOLUTE)
                if(EXISTS ${ULB_BIN_PREFIX}/${ILBINL})
                    set_target_properties(${target} PROPERTIES ${CONF_PROP_FULL} ${ULB_BIN_PREFIX}/${ILBINL})
                    message("${target} IMPORTED_LOCATION located.")
                else()
                    message("${target} IMPORTED_LOCATION could not be found, this may cause issues later...")
                endif()
            endif()

            getConfigurableProperty(ILBINL ${target} IMPORTED_IMPLIB NO_CRASH)
            if(NOT (${CONF_PROP_FULL} STREQUAL NOTFOUND) AND NOT EXISTS ${ILBINL})
                message("${target} IMPORTED_IMPLIB does not exist, attempting to locate...")
                get_filename_component(ULB_BIN_PREFIX "${ULB_BIN_PREFIX}" ABSOLUTE)
                if(EXISTS ${ULB_BIN_PREFIX}/${ILBINL})
                    set_target_properties(${target} PROPERTIES ${CONF_PROP_FULL} ${ULB_BIN_PREFIX}/${ILBINL})
                    message("${target} IMPORTED_IMPLIB located.")
                else()
                    message("${target} IMPORTED_IMPLIB could not be found, this may cause issues later...")
                endif()
            endif()
        endif()
...
macro(installUsedLibrary)
    if(${target_type} STREQUAL INTERFACE_LIBRARY)
        message("Target \"${target}\" is an interface library, no libs will be installed")
    else()
    ...
    if(ULB_HEADER_ROOT_DIR OR ULB_HEADER_INCLUDE_DIRS OR ULB_HEADER_INCLUDE_FILES)
        ...
        if(NOT ${target}_CMAKE_FILE)
            ...
            set(${target}_CMAKE_FILE ${CMAKE_BINARY_DIR}/${ULB_PREFIX}${target_name}Config.cmake)
            ...
            file(WRITE ${${target}_CMAKE_FILE} "
                ...
                set_target_properties(${target} PROPERTIES
                    INTERFACE_INCLUDE_DIRECTORIES \"\${PACK_PREF_DIR}/${CMAKE_INSTALL_INCLUDEDIR}/${ULB_HEADER_ROOT_DIR}\"
                    ${IMPLIB_INSTALL_LINE}
...
function(findLibrary name)
    cmake_parse_arguments(CSL "REQUIRED;QUIET;GLOBAL;NO_CONFIG;REQUIRE_TARGET" "DLL;LIB;DLLD;LIBD;FILE;FILED;NAME;FIND_HEADER;HEADER_DIR;ALT" "DEPENDENCIES;COMPONENTS;HINTS" ${ARGN})

    if(CSL_COMPONENTS)
        set(CSL_COMPONENTS2 ${CSL_COMPONENTS})
        list(POP_FRONT CSL_COMPONENTS2 first_obj)
        set(first_obj ::${first_obj})
    endif()
    testPackage(${name} "${CSL_ALT}" "${first_obj}")
    if(${${name}__FOUND})
        set(FPHSA_NAME_MISMATCHED 1)
        if(CSL_NO_CONFIG)
            find_package(${FOUND_DEP_NAME} REQUIRED ${CSL_COMPONENTS})
        else()
            find_package(${FOUND_DEP_NAME} REQUIRED ${CSL_COMPONENTS} CONFIG)
        endif()
        unset(FPHSA_NAME_MISMATCHED)
        #        if(TARGET "${name}" OR TARGET "${NAME}" OR TARGET "${ALT}")
        message("${name} FOUND")
        return()
        #        endif()
    endif()
    ...
    if(PKG_CONFIG_FOUND AND ${PKG_CONFIG_FOUND})
        pkg_search_module(${name} ${c_name})
        if(${name}_MODULE_NAME)
            message("${name} NOT FOUND -> PKG (${t_name})")
            if(CSL_GLOBAL)
            ...
            if(NOT WIN32 AND ${name}_LINK_LIBRARIES)
                set(CPY_LL ${${name}_LINK_LIBRARIES})
                list(POP_FRONT CPY_LL first_obj)
                set_property(TARGET ${t_name} PROPERTY IMPORTED_LOCATION "${first_obj}")
            endif()
...
macro(findWithHeaderInternal)
    set(HINTS2 "")
    foreach(hint ${CSL_HINTS})
        list(APPEND HINTS2 ${hint})
        list(APPEND HINTS2 ${hint}/include)
    endforeach()
    find_path(${name}_INCLUDE_DIR ${CSL_FIND_HEADER} HINTS ${HINTS2} NO_CACHE)
    set(${name}_INCLUDE_DIR ${${name}_INCLUDE_DIR}/${CSL_HEADER_DIR})
    set(HINTS2 "")
    foreach(hint ${CSL_HINTS})
        list(APPEND HINTS2 ${hint})
        list(APPEND HINTS2 ${hint}/lib)
    endforeach()
    if(CSL_FILE)
        find_library(${name}_LIBRARY ${CSL_FILE} HINTS ${HINTS2} NO_CACHE)
        if(${${name}_LIBRARY} STREQUAL ${name}_LIBRARY-NOTFOUND)
            find_library(${name}_LIBRARY ${name} HINTS ${HINTS2} NO_CACHE)
        endif()
    else()
        find_library(${name}_LIBRARY ${name} HINTS ${HINTS2} NO_CACHE)
    endif()
...
macro(checkImportedDLLInternal)
    if(WIN32)
        if(NOT ${name}_LIBRARY)
            ...
            set_property(TARGET ${t_name} PROPERTY IMPORTED_IMPLIB ${${name}_LIBRARY})
        endif()
...
function(createTarget name library include_dir)
    cmake_parse_arguments(CTA "GLOBAL" "" "" ${ARGN})
    get_filename_component(${name}_LIB_FOLDER ${library} DIRECTORY)
    list(APPEND BINZ ${${name}_LIB_FOLDER}/../bin)
    ...
    set_property(TARGET ${name} PROPERTY INTERFACE_INCLUDE_DIRECTORIES "${include_dir}")
    set_property(TARGET ${name} PROPERTY INTERFACE_LINK_LIBRARIES "${library}")

    checkImportedDLLInternal()
endfunction()

function(getRemoteLibrary name)
    cmake_parse_arguments(GRL "NO_BUILD;NO_GENERATOR" "GIT_REPO;GIT_TAG;FILE;FIND;SOURCE_ROOT" "BUILD_ARGS" ${ARGN})
    if(GRL_FILE)
    ...
    if(NOT ${name}_GIT_POPULATED OR NOT ${${name}_GIT_POPULATED})
        ...
        set(${name}_INSTALL_DIR ${${name}_BINARY_DIR}/installdir CACHE PATH "" FORCE)
        if(GRL_NO_BUILD)
            return()
        endif()
        set(CMAKE_DEP_ARGS "")
        ...
        list(APPEND CMAKE_DEP_ARGS -DCMAKE_INSTALL_PREFIX=${${name}_INSTALL_DIR})
        if(NOT GRL_NO_GENERATOR)
            if(CMAKE_GENERATOR)
        ...
        execute_process(COMMAND ${CMAKE_COMMAND} ${${name}_SOURCE_DIR}/${GRL_SOURCE_ROOT} ${CMAKE_DEP_ARGS} WORKING_DIRECTORY ${${name}_BINARY_DIR})
...
function(useRemoteSource name new_name)
    cmake_parse_arguments(URS "CLOSED" "PUBLIC_HEADER_DIR;ROOT" "PUBLIC_HEADER_FILES;PRIVATE_HEADER_FILES;SOURCE_FILES;REPLACE;HEADER_REPLACE" ${ARGN})
    ...
    set(${new_name}_ROOT_FOLDER ${ROOT_FOLDER} PARENT_SCOPE)

    if(${name}_${new_name}_SOURCE_POPULATED) #Cmake does not break off when first part of an "AND" is false...
        if(${${name}_${new_name}_SOURCE_POPULATED} AND (NOT EXISTS ${ROOT_FOLDER}))
            set(${name}_${new_name}_SOURCE_POPULATED FALSE CACHE BOOL "" FORCE)
        endif()
    endif()
    ...
    if(NOT ${name}_${new_name}_SOURCE_POPULATED OR NOT ${${name}_${new_name}_SOURCE_POPULATED})
        set(${name}_${new_name}_SOURCE_POPULATED TRUE CACHE BOOL "" FORCE)
        if(NOT URC_DEST_NAMES_LIST)
            delayDefer(replaceRemoteSourceFiles)
        endif()
        list(APPEND URC_DEST_NAMES_LIST "${new_name}")

        foreach(repl ${URS_REPLACE})
            list(APPEND ${new_name}_REPLACE_FILES_LIST "${repl}")
        endforeach()
        foreach(repl ${URS_HEADER_REPLACE})
            list(APPEND ${new_name}_HEADER_REPLACE_FILES_LIST "${repl}")
        endforeach()

        set(${new_name}_ALL_FILES_LIST ${${new_name}_ALL_FILES_LIST} PARENT_SCOPE)
        set(${new_name}_REPLACE_FILES_LIST ${${new_name}_REPLACE_FILES_LIST} PARENT_SCOPE)
        set(${new_name}_HEADER_REPLACE_FILES_LIST ${${new_name}_HEADER_REPLACE_FILES_LIST} PARENT_SCOPE)
        set(URC_DEST_NAMES_LIST ${URC_DEST_NAMES_LIST} PARENT_SCOPE)
    endif()
...
macro(subInstallRemoteSourceCopy subdir subdir2 file)
    if(NOT (${file} IN_LIST ALL_FILES))
        ...
        list(APPEND ${new_name}_ALL_FILES_LIST "${DEST}/${FNAME}")
...
function(replaceRemoteSourceFiles)
    foreach(new_name ${URC_DEST_NAMES_LIST})
        message("Handling: ${new_name}")
        foreach(file ${${new_name}_ALL_FILES_LIST})
            ...
            foreach(replaces ${URC_REPLACE_FILES_LIST})
                ...
                message("repl in ${file}: ${now} -> ${new}")
                string(REPLACE "${now}" "${new}" filedata "${filedata}")
            endforeach()
            foreach(replaces ${${new_name}_REPLACE_FILES_LIST})
                string(REPLACE "|" ";" replaces "${replaces}")
                list(POP_FRONT replaces now new)
                message("replD in ${file}: ${now} -> ${new}")
                string(REPLACE "${now}" "${new}" filedata "${filedata}")
            endforeach()
            if(((".h" STREQUAL "${file_ext}") OR (".hpp" STREQUAL "${file_ext}")) AND (${file} MATCHES "^${${new_name}_ROOT_FOLDER}/include"))
                foreach(replaces ${${new_name}_HEADER_REPLACE_FILES_LIST})
                    string(REPLACE "|" ";" replaces "${replaces}")
                    list(POP_FRONT replaces now new)
                    message("replD in ${file}: ${now} -> ${new}")
                    string(REPLACE "${now}" "${new}" filedata "${filedata}")
...
function(testPackage name name_alt fo)
    set(FPHSA_NAME_MISMATCHED 1)
    set(${name_alt}_FOUND 0)
    set(${name}_FOUND 0)
    find_package(${name} CONFIG QUIET)
    if(${${name}_FOUND} OR ${${name_alt}_FOUND} OR TARGET ${name}${fo} OR TARGET ${name_alt}${fo})
        if(TARGET ${name}${fo} OR TARGET ${name_alt}${fo})
            set(${name}_FOUND TRUE PARENT_SCOPE)
        endif()
        set(${name}__FOUND TRUE PARENT_SCOPE)
        set(FOUND_DEP_NAME ${name} PARENT_SCOPE)
        unset(FPHSA_NAME_MISMATCHED)
        return()
    elseif(CSL_NO_CONFIG)
    ...
    if(NOT ("${name_alt}" STREQUAL ""))
        find_package(${name_alt} QUIET)
        if(${${name}_FOUND} OR ${${name_alt}_FOUND} OR TARGET ${name}${fo} OR TARGET ${name_alt}${fo})
            if(TARGET ${name}${fo} OR TARGET ${name_alt}${fo})
                set(${name_alt}_FOUND TRUE PARENT_SCOPE)
            endif()
            set(${name}__FOUND TRUE PARENT_SCOPE)
            set(FOUND_DEP_NAME ${name_alt} PARENT_SCOPE)
            unset(FPHSA_NAME_MISMATCHED)
            return()
        endif()
    endif()
...
function(getConfigurableProperty out_name dep_name prop_name)
    cmake_parse_arguments(CP "NO_CRASH" "" "" ${ARGN})
    ...
    if(NOT ("${TPROPS}" STREQUAL TPROPS-NOTFOUND))
        ...
        if(NOT (${${out_name}} STREQUAL ${out_name}-NOTFOUND))
            set(CONF_PROP_FULL ${prop_name}_${ICTY} PARENT_SCOPE)
    ...
    if(${${out_name}} STREQUAL ${out_name}-NOTFOUND)
        if(CP_NO_CRASH)
            set(CONF_PROP_FULL NOTFOUND PARENT_SCOPE)
            return()
        else()
            message(FATAL_ERROR "Failed to find property ${prop_name} on: ${dep_name}")
        endif()
    endif()
    set(CONF_PROP_FULL ${prop_name} PARENT_SCOPE)
    set(${out_name} ${${out_name}} PARENT_SCOPE)
endfunction()