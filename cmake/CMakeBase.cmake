...
function(createSharedLibrary name)
    cmake_parse_arguments(CSL "USE;PUBLIC;GLOBAL" "DLL;LIB;DLLD;LIBD;FILE;FILED;INSTALL;NAME;FIND_HEADER;HEADER_DIR" "DEPENDENCIES;COMPONENTS" ${ARGN})
    if(CSL_PUBLIC)
        set(PUB PUBLIC)
    else()
        set(PUB PRIVATE)
...
macro(findWithHeaderInternal)
    find_path(${name}_INCLUDE_DIR ${CSL_FIND_HEADER} REQUIRED NO_CACHE)
    set(${name}_INCLUDE_DIR ${${name}_INCLUDE_DIR}/${HEADER_DIR})
    if(CSL_FILE)
        find_library(${name}_LIBRARY ${CSL_FILE} NO_CACHE)
    endif()
    if((NOT CSL_FILE) OR (${name}_LIBRARY STREQUAL ${name}-NOTFOUND))
        find_library(${name}_LIBRARY ${name} REQUIRED NO_CACHE)
    endif()
    ...
    set_property(TARGET ${name} PROPERTY INTERFACE_INCLUDE_DIRECTORIES "${${name}_INCLUDE_DIR}")
    set_property(TARGET ${name} PROPERTY INTERFACE_LINK_LIBRARIES "${${name}_LIBRARY}")

    checkImportedDLLInternal()
    message("${name} FOUND /W HEADERS")
endmacro()
